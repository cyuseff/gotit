...
module.exports.localLogin = function(req, res, next){
	passport.authenticate('local-login', function(err, user, info) {
		...
		return hh.sendJsonResponse(res, 200, { user: user.getPublicUser(), token:user.token });
...
module.exports.localStrategy = function(req, res) {
  if(req.body.email && req.body.password) {
		if(req.body.password === req.body.confirm_password) {
				...
				.exec(function(err, user) {
					...
					if(user) {
						if(user.local.email === req.body.email) {
							...
							return hh.sendJsonResponse(res, 400, {error: 'User already exits.'});
						} else {
							//Merge account strategy!
							return hh.sendJsonResponse(res, 400, {error: 'This email has been used to create another account, do you want to merge?'});
						}
					}
						...
						user.save(function(err){
							...
							user.generateToken(function(token){
								...
								user.save(function(err){
									...
									//return the new user
									hh.sendJsonResponse(res, 200, { user: user.getPublicUser(), token:user.token });
			...
			hh.sendJsonResponse(res, 400, {error:'Passwords don\'t match.'});
		}

	} else {
		hh.sendJsonResponse(res, 400, {error:'Email and password are required.'});
	}
